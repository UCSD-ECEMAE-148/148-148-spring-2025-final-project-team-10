...
import logging
import argparse
from location_tag import ESP32_Tag_Loc
#
...
def drive(cfg, use_joystick=False, camera_type='single'):
    ...
    # esp32_tag_loc_tracker = ESP32_Tag_Loc(args_esp32_imu)
    esp32_tag_loc_tracker = ESP32_Tag_Loc(cfg)
    esp32_tag_loc_tracker.start()
    ...
    # def generate_path_to_target(
    #     target_x, 
    #     target_y, 
    #     waypoints_per_meter = cfg.WAYPOINTS_PER_METER, 
    #     pid_throttle = cfg.PID_THROTTLE,
    #     filename='donkey_path.csv'
    # ):
    #     """
    #     Linearly interpolate a path from (0, 0) to (target_x, target_y)
    #     with a given number of waypoints.
    #     """
    #     num_waypoints = waypoints_per_meter * int(np.sqrt(target_x**2 + target_y**2))
    #     x_points = np.linspace(target_x, 0, num=num_waypoints)
    #     y_points = np.linspace(target_y, 0, num=num_waypoints)
    #     throttle_array = np.zeros(num_waypoints) + pid_throttle
    #     # last 10% waypoints, reduce throttle linearly to 0
    #     num_waypoints_decelerate = int(0.1 * num_waypoints)
    #     throttle_array[-num_waypoints_decelerate : ] = np.linspace(pid_throttle, 0, num = num_waypoints_decelerate)
        
    #     with open(filename, 'w', newline='') as csvfile:
    #         writer = csv.writer(csvfile)
    #         for x, y, throttle in zip(x_points, y_points, throttle_array):
    #             writer.writerow([x, y, throttle])
    ...
    def generate_path_to_target(
        ...
        tag_y, 
        waypoints_per_meter = cfg.WAYPOINTS_PER_METER, 
        path_curve_factor = cfg.PATH_CURVE_FACTOR,
        pid_throttle = cfg.PID_THROTTLE,
        pid_throttle_decelerate = cfg.PID_THROTTLE_DECELERATE,
        filename = cfg.PATH_FILENAME
    ):
        """
        Generate a path to target location in a curved path.
        """
        r = np.sqrt(tag_x**2 + tag_y**2)
        theta = np.arctan2(tag_y, tag_x)
        a = 2 * tag_x * tag_y / ((r ** 3) * path_curve_factor)
        rot_matrix = np.array([[np.cos(theta), -np.sin(theta)], [np.sin(theta), np.cos(theta)]])
        
        num_waypoints = waypoints_per_meter * int(np.sqrt(tag_x**2 + tag_y**2))
        x = np.linspace(0, r, num = num_waypoints)
        # reverse x
        x = x[::-1]
        y = -a * x * (x - r)
        ...
        x = x.reshape(-1, 1)
        y = y.reshape(-1, 1)
        xy_cat = np.concatenate((x, y), axis = 1).T
        xy_cat_rotated = rot_matrix @ xy_cat
        x_rotated = xy_cat_rotated[0, :]
        y_rotated = xy_cat_rotated[1, :]
        
        throttle_array = np.zeros(num_waypoints) + pid_throttle
        # last 10% waypoints, reduce throttle linearly to 0
        num_waypoints_decelerate = int(0.1 * num_waypoints)
        if num_waypoints_decelerate > 0:
            throttle_array[-num_waypoints_decelerate : ] = np.linspace(pid_throttle, 0, num = num_waypoints_decelerate)
        
        with open(filename, 'w', newline='') as csvfile:
            writer = csv.writer(csvfile)
            for x, y, throttle in zip(x_rotated, y_rotated, throttle_array):
                writer.writerow([x, y, throttle])
                
        print(f"Path saved to {filename}")
        
        # coordinates.csv contains the live coordinates, need to clear it everytime we save a path
        if os.path.exists('coordinates.csv'):
            with open('coordinates.csv', 'w', newline='') as csvfile:
                # Truncate file to clear contents
                csvfile.truncate(0)
            print("Cleared coordinates.csv")
        
    
    def save_path_esp32():
        esp32_tag_loc_tracker.reset_origin()
        tag_x, tag_y = esp32_tag_loc_tracker.get_location()
        generate_path_to_target(tag_x, tag_y)
    ...
    if cfg.SAVE_PATH_BTN:
        ...
        if cfg.SAVE_PATH_BTN.startswith("web/w"):
            ...
            V.add(Lambda(lambda: save_path_esp32()), run_condition=cfg.SAVE_PATH_BTN)
        elif have_joystick:
            ctr.set_button_down_trigger(cfg.SAVE_PATH_BTN, save_path_esp32)
    ...
    if cfg.RESET_ORIGIN_BTN:
        ...
        if cfg.RESET_ORIGIN_BTN.startswith("web/w"):
            ...
            ctr.set_button_down_trigger(cfg.RESET_ORIGIN_BTN, esp32_tag_loc_tracker.reset_origin)
    ...
    V.start(rate_hz=cfg.DRIVE_LOOP_HZ, 
        ...
        esp32_tag_loc=esp32_tag_loc_tracker)
