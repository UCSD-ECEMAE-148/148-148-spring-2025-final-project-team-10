...
class Vehicle:
    ...
    def start(self, rate_hz=10, max_loop_count=None, verbose=False, esp32_tag_loc = None):
        """
        ...
        try:
            ...
            loop_count = 0
            print("Here in the start function")
            
            coords_path = os.path.join('/home/jetson/projects/', 'esp32-loc', 'coordinates.csv')
            if not os.path.exists(coords_path):
                temp_file = open(coords_path, 'w', newline='')
                writer = csv.writer(temp_file)
                writer.writerow(['x', 'y'])  # Write header
                temp_file.close()
            
            # Open file for appending coordinates during loop
            counter_i = 0
            while self.on:
                ...
                if esp32_tag_loc is not None:
                    tag_x, tag_y = esp32_tag_loc.get_location()
                    print(f"tag_x: {tag_x}, tag_y: {tag_y}")
                    self.mem.put(['pos/x', 'pos/y'], [tag_x, tag_y])
                    counter_i += 1
                    if counter_i % 10 == 0:
                        # write the coordinates to ./../../esp32-loc/coordinates.csv
